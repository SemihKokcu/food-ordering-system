...
public class PaymentDomainServiceImpl  implements  PaymentDomainService{
    ...
    public PaymentEvent validateAndInitiatePayment(Payment payment, CreditEntry creditEntry, List<CreditHistory> creditHistories, List<String> failureMessages) {
        ...
        validatedCreditHistory(creditEntry,creditHistories,failureMessages);
    ...
    private void validateCreditEntry(Payment payment, CreditEntry creditEntry, List<String> failureMessages) {
        if (payment.getPrice().isGreatherThan(creditEntry.getTotalCreditAmount())){
            log.error("Customer with id: {} doesn't have enough credit for paymnet!",payment.getCustomerId().getValue());
    ...
    private void validatedCreditHistory(CreditEntry creditEntry, List<CreditHistory> creditHistories, List<String> failureMessages) {
        Money totalCreditHistory = getTotalHistoryAmount(creditHistories, TransactionType.CREDIT);
        Money totalDebitHistory = getTotalHistoryAmount(creditHistories, TransactionType.DEBIT);

        if (totalDebitHistory.isGreatherThan(totalCreditHistory)){
            log.error("Customer with id: {} doesn't have enough money credit according to credit history!",creditEntry.getCustomerId().getValue());
            failureMessages.add("Customer with id:"+creditEntry.getCustomerId().getValue()+" doesn't have enough money credit according to credit history!");
        }
